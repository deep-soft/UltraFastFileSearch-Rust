...
#[test]
fn repro_fuzz_crash_202ec8() {
    // 2026-08-17 nightly fuzzer find (crash-202ec813…): a 98-byte record
    // whose final `$FILE_NAME` attribute header is truncated mid-field, so
    // the value-offset read at bytes 20..22 of the attribute overran the
    // buffer ("range end index 100 out of range for slice of length 98").
    // Fixed by bounds-checked slicing in
    // `attribute_helpers::parse_file_name_full`.
    let data: [u8; 98] = [
        0x46, 0x49, 0x4C, 0x45, 0x4E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x31, 0xD1, 0xD1, 0x21, 0xD1, 0xD1, 0x00, 0x31,
        0xD1, 0xD1, 0x21, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1, 0x00, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1,
        0xD1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0xD1, 0xD1, 0xD1, 0x00, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1, 0xD1, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x46, 0x49, 0x4C, 0x45, 0x4E, 0xD1,
    ];
    core::hint::black_box(parse_record(&data, 0));
    core::hint::black_box(parse_record(&data, 5));
    core::hint::black_box(parse_record(&data, u64::MAX));
