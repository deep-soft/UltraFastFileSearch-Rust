...
fn test_daemon_ipc_all_methods() {
    ...
    assert!(
        ...
        "status should have uptime: {resp}"
    );
    ...
    assert!(
        resp.contains("\"drives\":[]"),
        "drives should be empty: {resp}"
    );
    ...
    assert!(
        resp.contains("\"rows\":[]"),
        "search should be empty: {resp}"
    );
    assert!(
        resp.contains("\"records_scanned\":0"),
        "should scan 0: {resp}"
    );
    ...
    assert!(
        resp.contains("-32601"),
        "should be method not found: {resp}"
    );
    ...
    assert!(
        ...
        "keepalive should return ok: {resp}"
    );
    ...
    if !nonce.is_empty() {
        let resp = rpc(
            ...
            &mut reader,
            &format!(
                r#"{{"jsonrpc":"2.0","id":8,"method":"shutdown","params":{{"nonce":"{nonce}"}}}}"#
            ),
        );
        assert!(
            resp.contains("shutting down") || resp.contains("\"ok\":true"),
...
/// Real-data integration test: loads G_mft.bin fixture, verifies actual search
/// results.
#[test]
fn test_real_data_search() {
    ...
    let mut ready = false;
    for _ in 0..80 {
        // 8 seconds max (MFT parse can take time)
        std::thread::sleep(Duration::from_millis(100));
        if sock.exists() {
            ready = true;
            break;
    ...
    assert!(
        resp.contains("\"letter\":\"G\""),
        "should have drive G: {resp}"
    );
    assert!(
        !resp.contains("\"records\":0"),
        "should have records > 0: {resp}"
    );
    ...
    assert!(
        !resp.contains("\"rows\":[]"),
        "search * should return results: {resp}"
    );
    assert!(
        resp.contains("\"name\""),
        "results should have name field: {resp}"
    );
    assert!(
        resp.contains("\"path\""),
        "results should have path field: {resp}"
    );
    assert!(
        resp.contains("\"records_scanned\""),
        "should have records_scanned: {resp}"
    );
    ...
    assert!(
        resp.contains("\"rows\""),
        "search *.txt should have rows field: {resp}"
    );
    assert!(
        resp.contains("\"duration_ms\""),
        "should have duration: {resp}"
    );
    ...
    assert!(
        resp.contains("\"ready\"") || resp.contains("\"Ready\""),
    ...
    if !nonce.is_empty() {
        let _ = rpc(
            ...
            &mut reader,
            &format!(
                r#"{{"jsonrpc":"2.0","id":99,"method":"shutdown","params":{{"nonce":"{nonce}"}}}}"#
            ),
        );
    } else {
        let _ = daemon.kill();
...
fn test_benchmark_round_trip_latency() {
    ...
    assert!(
        avg.as_millis() < 15,
        "average latency {:?} exceeds 15ms target",
        avg
    );
...
fn test_concurrent_clients() {
    ...
    if !nonce.is_empty() {
        ...
        let _ = rpc(
            ...
            reader,
            &format!(
                r#"{{"jsonrpc":"2.0","id":999,"method":"shutdown","params":{{"nonce":"{nonce}"}}}}"#
            ),
        );
    } else {
        let _ = daemon.kill();
