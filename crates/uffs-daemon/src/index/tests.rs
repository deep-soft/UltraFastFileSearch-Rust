...
fn preset_overview_returns_multiple_results() {
    ...
    let results = IndexManager::run_aggregations(&index, &specs, Vec::new());
...
fn count_returns_total_records() {
    let index = test_index();
    let results = IndexManager::run_aggregations(&index, &[spec("count")], Vec::new());
...
fn query_predicates_forwarded_to_drilldown() {
    ...
    let results = IndexManager::run_aggregations(&index, &specs, predicates);
    let result = results
        .iter()
        .find(|r| r.kind == "buckets")
        .expect("should have buckets");
