...
use super::parity_flags;
use super::{vb, vi, vs, vu, vu32, CppFooterContext, ParityContext};

/// Parity-compat CSV header (25 columns, matching legacy baseline).
pub(super) const PARITY_HEADER: &[&str] = &[
    "Path", "Name", "Path Only", "Size", "Size on Disk", "Created",
    "Last Written", "Last Accessed", "Descendants", "Read-only", "Archive",
    "System", "Hidden", "Offline", "Not content indexed file", "No scrub file",
    "Integrity", "Pinned", "Unpinned", "Directory Flag", "Compressed",
    "Encrypted", "Sparse", "Reparse", "Attributes",
];

/// Boolean columns in parity order (matches `PARITY_HEADER[9..25]`).
const PARITY_BOOL_FLAGS: &[u32] = &[
    parity_flags::READONLY, parity_flags::ARCHIVE, parity_flags::SYSTEM,
    parity_flags::HIDDEN, parity_flags::OFFLINE, parity_flags::NOT_INDEXED,
    parity_flags::NO_SCRUB, parity_flags::INTEGRITY, parity_flags::PINNED,
    parity_flags::UNPINNED, parity_flags::DIRECTORY, parity_flags::COMPRESSED,
    parity_flags::ENCRYPTED, parity_flags::SPARSE, parity_flags::REPARSE,
];

/// Write parity-compat 25-column CSV output from `SearchRow` data.
pub(super) fn write_parity<W: Write>(
    writer: &mut W, rows: &[Value], separator: &str, quote: &str,
    ctx: &ParityContext<'_>,
) -> Result<()> {
    let mut header = String::with_capacity(512);
    for (idx, col) in PARITY_HEADER.iter().enumerate() {
        if idx > 0 { header.push_str(separator); }
...
fn write_parity_row(
    buf: &mut String, row: &Value, sep: &str, quote: &str, ctx: &ParityContext<'_>,
) {
    ...
    buf.push_str(quote);
    buf.push_str(&path);
    if is_dir && !path.ends_with('\\') { buf.push('\\'); }
    buf.push_str(quote);

    // 1: Name (quoted, empty for dirs)
    buf.push_str(sep); buf.push_str(quote);
    if !is_dir { buf.push_str(&name); }
    buf.push_str(quote);
    ...
    if is_dir {
        buf.push_str(&path);
        if !path.ends_with('\\') { buf.push('\\'); }
    } else if let Some(pos) = path.rfind('\\') {
        if let Some(slice) = path.get(..=pos) { buf.push_str(slice); }
    } else {
        buf.push_str(&path);
    }
    buf.push_str(quote);
    ...
    buf.push_str(sep);
    push_u64(buf, if is_dir { vu(row, "treesize") } else { vu(row, "size") });

    // 4: SizeOnDisk (tree_allocated for dirs)
    buf.push_str(sep);
    push_u64(buf, if is_dir { vu(row, "tree_allocated") } else { vu(row, "allocated") });
...
#[expect(clippy::cast_sign_loss, reason = "timestamps are non-negative in practice")]
fn append_datetime_tz(buf: &mut String, unix_us: i64, tz_offset_secs: i32) {
    ...
    let _ok = write!(buf, "{year:04}-{month:02}-{day:02} {:02}:{:02}:{:02}",
