...
//!
//! Defines [`TuiColumn`] — the set of columns a search UI can display —
//! along with header labels, sort-column mapping, and the `--columns` parser.

use super::backend::SortColumn;

/// A column that a search UI can display.
///
/// Full CLI-parity with `OutputColumn` in the CLI crate. Every column
