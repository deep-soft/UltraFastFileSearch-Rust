...
pub struct Win32MountPoint {
    /// The `Directory` property contains the WMI path to the associated directory
    /// For example: `Win32_Directory.Name="F:\\"`
    Directory: String,
    ...
    /// For `Win32_MountPoint`, this might be `Win32_MountPoint`.
    ...
    /// It shows which class `Win32_MountPoint` inherits from.
...
pub fn query_mount_point() -> Result<Vec<Win32MountPoint>, UFFSError> {
    ...
    let results: Vec<Win32MountPoint> = wmi_con.raw_query(query)
