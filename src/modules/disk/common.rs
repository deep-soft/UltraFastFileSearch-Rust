...
//             // Fetch the number of files and directories for the current drive
//             let (num_files, num_dirs) = get_file_dir_len(&disk.root_path).await.map_err(|e| {
//                 UFFSError::DirectoryReadError // Convert the error type
//             })?;
// 
//             // Update only the fields that need to be updated (num_files and num_dirs)
//             disk.num_files = num_files;
