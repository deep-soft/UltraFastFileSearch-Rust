...
> **Source:** [`LOG/Output_cache_newer:580-625`](../../LOG/Output_cache_newer). 7 drives loaded, bench runs on C: + D: only for apples-to-apples against Everything's same-drive constraint. n = 30 rounds per cell, file sink (`--out` / `-export-csv`). p50 and p95 columns from the same distribution.
...
\* C:prefix is a 100-round interleaved re-bench (source: [`LOG/Output_cache_newest:310-412`](../../LOG/Output_cache_newest)): UFFS 94.5 ms vs ES 95.7 ms, UFFS 0.99× — the 30-round pass above landed on an unlucky disk-busyness tick and showed 1.02×. Both numbers are published; the 100-round number is authoritative.
...
> **Source:** [`LOG/Output_cache_latest`](../../LOG/Output_cache_latest), measured by [`scripts/windows/cold-parity-per-drive.ps1`](../../scripts/windows/cold-parity-per-drive.ps1). Drive G (15 k-record USB) excluded as not representative. Per drive, sequential methodology preserved from the v0.4.106 historical snapshot: purge UFFS cache → stop daemon → Rust COLD → C++ on same drive with OS page cache warmed by Rust's just-finished read.
...
> **Sources:** [`LOG/Output_cache_newest:933-1044`](../../LOG/Output_cache_newest) (drive-scale sweep), [`LOG/Output_cache_new:315-412`](../../LOG/Output_cache_new) (memory), [`LOG/Output_cache_newest:1263-1278`](../../LOG/Output_cache_newest) (full-scan export).
...
Source: [`LOG/Output_cache_new:413-479`](../../LOG/Output_cache_new), n = 10 each, JSON output via `--out`:
...
> **Source:** [`LOG/Output_cache_newest:573-707`](../../LOG/Output_cache_newest). 7-drive daemon, 26.1 M records, n = 30 rounds, file sink.
...
Every number in this report comes from one of five raw logs, all checked into `LOG/` on the test machine:

- [`LOG/Output_cache_newer`](../../LOG/Output_cache_newer) — cross-tool HOT comparison vs Everything (12/12 table, §1).
- [`LOG/Output_cache_newest`](../../LOG/Output_cache_newest) — forensic 100-round re-bench, drive-scale sweep, `--sort path` pin, full-scan export, direct-redirect stdout.
- [`LOG/Output_cache_new`](../../LOG/Output_cache_new) — daemon memory + startup + aggregations.
- [`LOG/Output_cache_latest`](../../LOG/Output_cache_latest) — per-drive Rust-vs-C++ parity (§2).
- [`LOG/Output_cache`](../../LOG/Output_cache) — v0.5.62 baseline + CLI/API/MCP validation suites (100% pass on 729 tests across all three surfaces).
...
- **Benchmark methodology (full, internal):** [`docs/research/cross-tool-benchmark-analysis.md`](../research/cross-tool-benchmark-analysis.md) — the engineering-detail version of this report; currently kept internal (see `docs/benchmarks/methodology.md` for the public summary once published)
